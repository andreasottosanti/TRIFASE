# Run additional simulation study 1 ----
...
dir_path_scenario<-"SIMULATION_STUDIES/SIMULATION_RESULTS/Add_sim_study_1/SCENARIOS/"
dir_path_genData<-"SIMULATION_STUDIES/SIMULATION_RESULTS/Add_sim_study_1/GENERATED_DATA/"
dir_path_estimate<-"SIMULATION_STUDIES/SIMULATION_RESULTS/Add_sim_study_1/ESTIMATE_RUNS/"
dir_path_extracted<-"SIMULATION_STUDIES/SIMULATION_RESULTS/Add_sim_study_1/EXTRACT_RESULTS/"
dir_path_parameters<-"SIMULATION_STUDIES/SIMULATION_RESULTS/Add_sim_study_1/SET_PARAMETERS/"
...
nclust.pt.start <- c(12)
nclust.prot.start <- c(3,6)
tau.start <- c(3,6)
...
n.parameters<-(length(nclust.pt.start)*length(nclust.prot.start)*length(tau.start)*
                 length(phi.start)*length(est.upd.row)*length(est.upd.col)*length(est.tau))+  #96 combinations for TRIFASE (est.kernel=exponential)
  (length(nclust.pt.start)*length(nclust.prot.start))+ #2 combinations for Wang (est.kernel=none)
  (length(nclust.pt.start)*length(nclust.prot.start))  #2 combinations for k-means (est.kernel=km)
for(int in 1:n.scenari){
