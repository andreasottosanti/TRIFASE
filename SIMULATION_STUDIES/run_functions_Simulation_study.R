...
dir_path_scenario<-"SIMULATION_STUDIES/SIMULATION_RESULTS/Simulation_Study/SCENARIOS/"
dir_path_genData<-"SIMULATION_STUDIES/SIMULATION_RESULTS/Simulation_Study/GENERATED_DATA/"
...
dir_path_parameters<-"SIMULATION_STUDIES/SIMULATION_RESULTS/Simulation_Study/SET_PARAMETERS/"
...
n.parameters<-(length(nclust.pt.start)*length(nclust.prot.start)*length(tau.start)*
  length(phi.start)*length(est.upd.row)*length(est.upd.col)*length(est.tau))+  #192 combinations for TRIFASE (est.kernel=exponential)
  (length(nclust.pt.start)*length(nclust.prot.start))+ #8 combinations for Wang (est.kernel=none)
  (length(nclust.pt.start)*length(nclust.prot.start))  #8 combinations for k-means (est.kernel=km)
for(int in 1:n.scenari){
